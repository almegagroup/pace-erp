...
latest_menu as (
  ...
  where ms.menu_code in ('PROC_PI_LIST', 'PROC_PI_COUNT_ENTRY', 'PROC_PI_RECOUNT')
)
select
  lm.company_code,
  lm.user_code,
  lm.work_context_code,
  lm.work_context_name,
  lm.menu_code,
  lm.is_visible
from latest_menu lm
where lm.rn = 1
order by lm.company_code, lm.user_code, lm.menu_code;
