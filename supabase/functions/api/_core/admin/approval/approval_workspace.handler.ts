...
type ApprovalWorkspaceResource = {
...
function deriveAvailableActions(resourceCode: string) {
  if (resourceCode.endsWith("_APPLY")) return ["VIEW", "WRITE"];
  if (resourceCode.includes("_MY_REQUESTS")) return ["VIEW", "EDIT"];
  if (resourceCode.includes("_APPROVAL_INBOX")) return ["VIEW", "APPROVE"];
  if (resourceCode.includes("_APPROVAL_SCOPE_HISTORY")) return ["VIEW"];
  if (resourceCode.endsWith("_REGISTER")) return ["VIEW", "EXPORT"];
  return [...DEFAULT_ACTIONS];
}

function isApprovalWorkspaceResource(value: unknown): value is ApprovalWorkspaceResource {
...
export async function listApprovalWorkspaceHandler(
  _req: Request,
  ...
  try {
    ...
    const [
      ...
      { data: approverRows, error: approverError },
    ...
    const firstError =
      ...
      parentCompanyError ?? approverError ?? viewerError;

    if (firstError) {
      return errorResponse(
        "APPROVAL_WORKSPACE_LIST_FAILED",
        firstError.message,
        ctx.request_id,
      );
    }
    ...
    const resources = (resourceMapRows ?? [])
      ...
      .filter(isApprovalWorkspaceResource)
      .sort((left, right) =>
    ...
    return errorResponse(
      (err as Error).message || "APPROVAL_WORKSPACE_LIST_EXCEPTION",
      "approval workspace list exception",
      ctx.request_id,
    );
  }
}
