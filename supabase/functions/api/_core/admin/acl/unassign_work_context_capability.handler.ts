/*
 * File-ID: ID-9.7C
 * File-Path: supabase/functions/api/_core/admin/acl/unassign_work_context_capability.handler.ts
 * Gate: 9
 * Phase: 9
 * Domain: ACL
 * Purpose: Remove a capability pack from a work context
 * Authority: Backend
 */
...
export async function unassignWorkContextCapabilityHandler(
  req: Request,
  ctx: AdminContext,
): Promise<Response> {
  ...
  try {
    ...
    if (error) {
      return errorResponse(
        "WORK_CONTEXT_CAPABILITY_UNASSIGN_FAILED",
        error.message,
        requestId,
      );
    }
