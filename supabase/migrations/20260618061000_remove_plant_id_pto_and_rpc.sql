-- Migration: Remove plant_id from plant_transfer_order and update post_stock_movement RPC
-- Follows 20260618060000_remove_plant_id_om_sc.sql

-- ============================================================
-- 1. plant_transfer_order — drop source_plant_id / target_plant_id
-- Both replaced by source_company_id / target_company_id already present
-- ============================================================
ALTER TABLE erp_procurement.plant_transfer_order
  DROP CONSTRAINT IF EXISTS plant_transfer_order_source_plant_id_fkey,
  DROP CONSTRAINT IF EXISTS plant_transfer_order_target_plant_id_fkey;

ALTER TABLE erp_procurement.plant_transfer_order
  DROP COLUMN IF EXISTS source_plant_id,
  DROP COLUMN IF EXISTS target_plant_id;

-- ============================================================
-- 2. post_stock_movement — remove p_plant_id parameter
-- stock_document, stock_ledger, stock_snapshot no longer have plant_id
-- ============================================================
DROP FUNCTION IF EXISTS erp_inventory.post_stock_movement(
  text, date, date, text, uuid, uuid, uuid, uuid, uuid, numeric, text, numeric, text, text, uuid, uuid
);
