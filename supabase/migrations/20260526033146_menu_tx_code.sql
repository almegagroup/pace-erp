-- ============================================================
-- File-Path: supabase/migrations/20260526010000_menu_tx_code.sql
-- Purpose: Add tx_code (transaction code) to menu_master and
--          menu_snapshot for SAP-style Command Palette navigation
-- Authority: Backend
-- ============================================================

BEGIN;

-- 1. Add tx_code to menu_master
ALTER TABLE erp_menu.menu_master
  ADD COLUMN IF NOT EXISTS tx_code text UNIQUE;

COMMENT ON COLUMN erp_menu.menu_master.tx_code IS
'Short transaction code for SAP-style command navigation (e.g. OM01, UM02). Used as keyword in Command Palette search.';

-- 2. Add tx_code to menu_snapshot
ALTER TABLE erp_menu.menu_snapshot
  ADD COLUMN IF NOT EXISTS tx_code text;

COMMENT ON COLUMN erp_menu.menu_snapshot.tx_code IS
'Transaction code copied from menu_master at snapshot generation time.';

-- 3. Rebuild generate_menu_snapshot to carry tx_code into snapshot
...
IS 'Builds menu_snapshot from full SA registry or ACL precomputed_acl_view, including ancestor groups, descriptions, and tx_codes.';
