/*
 * Communication Automation — Phase 6 corrective migration.
 *
 * The original `save_automation_rule(..., p_status, ...)` let the caller pick
 * any lifecycle value on every save, which is how a normal "Save" on an
 * INACTIVE (or ACTIVE) rule could silently downgrade it back to DRAFT. This
 * migration removes lifecycle choice from the generic save path entirely and
 * makes each transition its own narrow, server-enforced function:
 *
 *   save_automation_rule(...)      -- new row -> DRAFT, existing row -> unchanged status
 *   activate_automation_rule(...)  -- new or existing row -> ACTIVE (explicit action only)
 *   set_automation_rule_status(...) -- ACTIVE -> INACTIVE only (explicit Deactivate)
 *
 * A client can no longer submit an arbitrary desired status on a save call.
 */

BEGIN;

DROP FUNCTION IF EXISTS erp_communication.save_automation_rule(
...
BEGIN
  ...
  IF p_rule_id IS NULL THEN
    ...
    INSERT INTO erp_communication.automation_rule (
      ...
      p_company_id, p_surface_enrollment_id, p_channel, pg_catalog.btrim(p_rule_name), v_status, NULLIF(pg_catalog.btrim(p_dataset_key), ''),
  ...
  ELSE
    ...
    SELECT rule.id, rule.version_no, rule.status INTO v_rule_id, v_version_no, v_status
    ...
    -- Lifecycle status is intentionally NOT part of this SET clause: a normal
    -- save always preserves whatever status the rule already has. Only
    -- activate_automation_rule()/set_automation_rule_status() may change it.
  ...
  RETURN QUERY SELECT v_rule_id, v_version_no, v_status;
END;
$function$;

CREATE OR REPLACE FUNCTION erp_communication.activate_automation_rule(
  p_rule_id uuid,
  p_company_id uuid,
  p_surface_enrollment_id uuid,
  p_channel text,
...
BEGIN
  ...
  IF p_rule_id IS NULL THEN
    ...
    INSERT INTO erp_communication.automation_rule (
      ...
      p_company_id, p_surface_enrollment_id, p_channel, pg_catalog.btrim(p_rule_name), 'ACTIVE', NULLIF(pg_catalog.btrim(p_dataset_key), ''),
      COALESCE(p_subject_template, ''), p_schedule_kind, p_schedule_time, p_schedule_timezone,
      COALESCE(p_weekly_days, ARRAY[]::smallint[]), p_monthly_day, p_skip_empty, 1,
      pg_catalog.now(), p_actor, pg_catalog.now(), p_actor
    ) RETURNING id, automation_rule.version_no INTO v_rule_id, v_version_no;
  ...
  RETURN QUERY SELECT v_rule_id, v_version_no, 'ACTIVE'::text;
END;
$function$;

-- Deactivate must only ever move a rule out of ACTIVE. Without this guard a
-- crafted call could move a DRAFT/INACTIVE rule "into INACTIVE" directly,
-- which is not a real transition (it never left DRAFT) and would defeat the
...
BEGIN
  ...
  SELECT rule.id, rule.version_no, rule.status INTO v_rule_id, v_version_no, v_current_status
  FROM erp_communication.automation_rule AS rule
  WHERE id = p_rule_id
    AND company_id = p_company_id
    AND surface_enrollment_id = p_surface_enrollment_id
  ...
  IF v_current_status <> 'ACTIVE' THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'COMMUNICATION_RULE_INVALID_TRANSITION';
  END IF;
...
ALTER FUNCTION erp_communication.save_automation_rule(
  uuid, uuid, uuid, text, text, text, text, text, time, text, smallint[], smallint,
  boolean, integer, jsonb, jsonb, uuid
) OWNER TO postgres;
ALTER FUNCTION erp_communication.activate_automation_rule(
  uuid, uuid, uuid, text, text, text, text, text, time, text, smallint[], smallint,
  boolean, integer, jsonb, jsonb, uuid
) OWNER TO postgres;
...
REVOKE ALL ON FUNCTION erp_communication.activate_automation_rule(
  uuid, uuid, uuid, text, text, text, text, text, time, text, smallint[], smallint,
  boolean, integer, jsonb, jsonb, uuid
) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION erp_communication.activate_automation_rule(
  uuid, uuid, uuid, text, text, text, text, text, time, text, smallint[], smallint,
  boolean, integer, jsonb, jsonb, uuid
) TO service_role;
