-- AC01 GRN Landed Cost Hub — real gap found live 2026-08-22, business owner
-- confirmed: Additional Duty/IGST (cost_type ADDITIONAL_DUTY_IGST) paid on
-- import is ITC-creditable, exactly like domestic purchase GST -- not a real
-- cost. Every OTHER duty type (Import Duty, Excise Duty, CST, Customs Edn
-- Cess, Duty Set-off, Entry Tax, Customs Duty) genuinely is a real cost and
-- correctly stays in Landed Cost. Duty-type cost lines never expose the
-- has_gst/gst_treatment UI at all (AC01Page.jsx's isDuty gate), so the
-- existing GST-backing-out mechanism (used for ad-hoc/per-UoM charges) never
-- applied here -- the entire entered amount was flowing into Landed Cost
-- unconditionally, including 100% of the IGST duty line, which should have
-- been excluded the same way GST is excluded everywhere else in this design
-- ("GST must never enter Landed Cost or WAR", locked 2026-08-21).
--
-- Fix: ADDITIONAL_DUTY_IGST's entire amount is now excluded from the net
-- (Landed Cost) figure -- the whole line IS the IGST itself, not a base+tax
-- blend, so there's no percentage to back out, just a full exclusion. The
-- gross (Payable) figure is deliberately UNCHANGED -- if this line is
-- assigned to a real party (Vendor/CHA/etc via party_type), that party still
-- needs to be paid the full amount; only its Landed Cost/WAR contribution
-- changes.
--
-- Signature is identical to the previous CREATE OR REPLACE (20260822100000)
-- -- no DROP FUNCTION needed here, only the net_amount calculation changes.
...
BEGIN
  ...
  LOOP
    ...
    -- Additional Duty/IGST on import is ITC-creditable, exactly like
    -- domestic purchase GST -- the entire line IS the IGST (no base+tax
    -- blend to back a percentage out of), so it's fully excluded from
    -- Landed Cost. Gross (Payable) is untouched -- still owed to whichever
    -- party this line is assigned to.
