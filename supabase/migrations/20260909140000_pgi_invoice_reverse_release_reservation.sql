-- PGI/Invoice REVERSE branch never released reservation_document back to
-- OPEN/PARTIAL, unlike the CREATE branch which sets it to FULLY_ISSUED.
-- Found 2026-09-09 while verifying dispatch_reco reversal symmetry (user
-- question: does AC10 correctly show both a dispatch and its reversal, and
-- is any OTHER summary field left un-subtracted on cancellation). The two
-- real cancelled invoices tested (9200000223/9200000224) happened to have
-- their parent Sales Orders also cancelled via a separate SO-cancel path
-- that cleaned up their reservations -- masking that complete_pgi_invoice_
-- action's own REVERSE branch never touches reservation_document at all.
-- If an invoice is ever reversed while its SO/STO stays ACTIVE, the
-- reservation would stay stuck at FULLY_ISSUED forever even though the
-- stock physically came back (P602), permanently misrepresenting that
-- line's availability. Fixed by mirroring CREATE's own reservation update,
-- scoped to exactly this invoice's own lines (sales_invoice_line.dc_line_id
-- -> delivery_challan_line.so_line_id/sto_line_id), moving issued_qty back
-- down and status back to OPEN/PARTIAL.
...
BEGIN
  ...
  ELSIF v_action = 'REVERSE' THEN
    ...
    -- Mirror CREATE's own reservation update in reverse: this invoice's own
    -- lines only (via dc_line_id), never another invoice group's lines on
    -- the same DC. issued_qty moves back down (never below 0); status
    -- drops to OPEN once fully un-issued, else stays PARTIAL.
    UPDATE erp_production.reservation_document rd
