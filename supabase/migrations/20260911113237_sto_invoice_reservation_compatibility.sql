-- Use invoice lines as the authoritative reservation quantities. Works with
-- existing API payloads and split lines without changing unrelated DOs.
...
BEGIN
  IF v_action = 'CREATE' THEN
    ...
    SET status = 'FULLY_ISSUED',
        issued_qty = (r->>'quantity')::numeric,
        last_updated_at = now(),
        last_updated_by = NULLIF(v_inv->>'posted_by', '')::uuid
    FROM jsonb_array_elements(COALESCE(p_context->'lines', '[]'::jsonb)) AS r
    WHERE rd.company_id = (v_inv->>'company_id')::uuid
      AND EXISTS (
        SELECT 1 FROM erp_procurement.delivery_challan_line own_line
        WHERE own_line.id = rd.dc_line_id AND own_line.dc_id = v_dc_id
          AND rd.source_line_id = COALESCE(own_line.so_line_id, own_line.sto_line_id)
      )
