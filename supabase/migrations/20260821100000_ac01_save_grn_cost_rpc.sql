-- AC01 GRN Landed Cost Hub — atomic save RPC.
-- Single transaction: GRN field update + landed_cost header get-or-create +
-- cost/deduction line replace + Base-UoM landed-cost-per-unit computation +
-- WAR recalculation (erp_inventory.recalculate_valuation_at_row), matching
-- CLAUDE.md §8D's post_document/single-RPC atomicity discipline — the
-- frontend's Save action calls this ONE function, never a sequence of
-- separate TS-side writes.
--
-- GST-exclusion rule (business-owner lock 2026-08-21): GST is never part of
-- Landed Cost/WAR. Per cost line, if gst_treatment='INCLUSIVE', the entered
-- amount is backed out via amount/(1+gst_rate/100) before it feeds the total;
-- if 'EXCLUSIVE' (or unset), the amount is used as-is.
--
-- Deduction lines with in_landed=true are added to the same total on the same
-- basis (their own amount, no GST treatment of their own per the locked
-- design — deductions are plain amount/percentage/round_off).
--
-- Base UoM rule (business-owner lock 2026-08-21): Landed Cost is always
-- computed on Base UoM — mirrors goods_receipt's own existing per_pack_qty
-- conversion (the same conversion createGRNFromLineHandler already applies
-- when po_uom differs from base_uom).
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'erp_procurement', 'erp_inventory', 'public'
AS $function$
DECLARE
  v_grn                RECORD;
  v_lc_id              uuid;
...
BEGIN
  ...
  -- 2. Get-or-create the landed_cost header for this GRN (one active DRAFT/POSTED
  --    header per GRN going forward — reuses the most recent one if present,
  --    matching the existing getLandedCostForGRN "latest wins" read pattern).
  ...
  -- 3. Replace cost lines (recalculative — each Save fully replaces the set,
  --    matching the business reality that bills arrive incrementally and the
  --    whole breakdown gets re-entered/re-confirmed each time, not diffed).
  ...
  LOOP
    ...
    IF v_line->>'gst_treatment' = 'INCLUSIVE' AND (v_line->>'gst_rate') IS NOT NULL AND (v_line->>'gst_rate')::numeric > 0 THEN
    ...
    INSERT INTO erp_procurement.landed_cost_line (
      ...
      entry_mode, percentage, gst_treatment, gst_rate
    ) VALUES (
  ...
  LOOP
    INSERT INTO erp_procurement.landed_cost_deduction_line (
      lc_id, deduction_type_id, amount, percentage, round_off, in_landed, created_by
    ) VALUES (
      v_lc_id,
  ...
  -- 5. Base-UoM landed cost per unit, then update the header total and (if
  --    the GRN has an actual stock posting) recalculate WAR.
...
COMMENT ON FUNCTION erp_procurement.save_ac01_grn_cost IS
  'AC01''s single-transaction Save: GRN fields + landed_cost lines/deductions + WAR recalculation. Never call the individual table writes or erp_inventory.recalculate_valuation_at_row separately from TypeScript for AC01 — this function is the one atomic entry point (CLAUDE.md §8D).';
