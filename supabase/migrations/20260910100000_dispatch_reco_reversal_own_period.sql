-- dispatch_reco / AC10: a PGI reversal must set off in the PERIOD IT
-- ACTUALLY HAPPENS IN, not retroactively erase the original dispatch's own
-- period. Found 2026-09-10 -- the REVERSE branch simply flipped
-- is_voided=true on the original row, and AC10's query excluded any
-- is_voided=true row entirely (`.eq("is_voided", false)`). That means an
-- August PGI reversed in September would vanish from BOTH months the
-- moment AC10 is re-pulled -- but August's AC10 was already used to bill
-- Asian Paints (a receivable recognized in August); the reversal is a real
-- September event (a payable/credit) and must show up THERE, not silently
-- erase August's history. Business owner: "August e receivable r
-- September e payable" -- this is a real accounting-period integrity bug.
--
-- Fix mirrors the ALREADY-LOCKED, already-verified PARTIAL_REVERSAL (PR19)
-- pattern used elsewhere in this same report (process_order_line_reco/
-- packing_order_line_reco): a reversal is a NEW, separate, negative-value
-- row dated to the reversal's own event date -- never an in-place edit of
-- the original.
--   1. The ORIGINAL row is no longer excluded from AC10 once voided --
--      it keeps showing in its own historical period forever (is_voided/
--      voided_at/voided_by stay as an informational audit marker only).
--   2. A NEW mirror row is inserted per original line: same material/
--      batch/process-order/packing-order/invoice identity (so it reads as
--      "this credits invoice X"), but Dispatch/Standard/Actual/AP-Approved
--      all NEGATED, and its own tally_invoice_date/invoice_date set to the
--      reversal's own date -- so it lands in the CORRECT (later) month's
--      AC10 pull via the report's existing tally_invoice_date range filter,
--      with zero query-shape changes needed there.
-- reversal_of_id lets a future report/audit trace a credit row back to the
-- exact original line it reverses.

ALTER TABLE erp_production.dispatch_reco
  ADD COLUMN reversal_of_id uuid REFERENCES erp_production.dispatch_reco(id);

CREATE INDEX ix_dispatch_reco_reversal_of ON erp_production.dispatch_reco (reversal_of_id) WHERE reversal_of_id IS NOT NULL;
...
BEGIN
  ...
  ELSIF v_action = 'REVERSE' THEN
    ...
    -- Void the original rows (audit marker only, no longer used to exclude
    -- them from AC10 -- see reco_data.handlers.ts), then insert a NEGATIVE
    -- mirror per voided row, dated to the reversal's own date, so AC10
    -- nets off in the PERIOD THE REVERSAL ACTUALLY HAPPENS IN.
    WITH voided AS (
