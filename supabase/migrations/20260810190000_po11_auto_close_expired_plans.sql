-- PO11 automatic month-end close (feasibility doc Section 35.13: "if the
-- implementation later supports an automatic month-end close at the
-- system's month boundary, that automatic close must produce the same
-- frozen snapshot result" as the manual close path).
--
-- This function mirrors closeProcurementPlanningMonthHandler +
-- loadWorkspaceRows in planning.handlers.ts, but only the pieces needed to
-- archive an *existing* plan's lines -- it does NOT re-run eligibility/
-- auto-include (that only matters for the live workspace view; by month-end
-- every eligible material already has a plan line from normal page usage
-- during the month). If the planning formula in loadWorkspaceRows ever
-- changes, this function must be updated to match, or the automatic close
-- will silently diverge from what a manual close would have produced.
  ...
  loop
    begin
      ...
      insert into erp_procurement.procurement_monthly_plan_archive_line (
        ...
        material_code_snapshot, material_name_snapshot, base_uom_code_snapshot,
      ...
      select
        ...
        -- derived safety = daily requirement * safety days
        ...
        -- derived replenishment = derived safety + daily requirement * (processing + lead)
    ...
    exception when others then
      -- Isolate one company's failure from the rest of the batch -- a
      -- problem archiving CMP003's plan must not block CMP006's close.
...
comment on function erp_procurement.auto_close_expired_procurement_plans() is
  'PO11 automatic month-end close (feasibility Section 35.13). Scheduled daily via pg_cron at 00:05 IST; closes any OPEN monthly plan whose plan_month has fully ended (IST calendar), archiving the same fields the manual Close Month action would. Safe to run more than once a day -- only picks up status=''OPEN'' rows.';

-- Runs daily at 00:05 IST (18:35 UTC the previous day) -- comfortably after
-- India midnight so the (now() at time zone 'Asia/Kolkata')::date check
-- inside the function has already rolled over to the new month.
select cron.schedule(
  'po11-auto-close-expired-plans',
  '35 18 * * *',
  $$select erp_procurement.auto_close_expired_procurement_plans();$$
);
