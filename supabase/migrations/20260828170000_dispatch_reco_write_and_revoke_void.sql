-- §133.14 Part B + §133.15 point 2 -- extends complete_pgi_invoice_action()
-- (last touched by 20260828150000_so02_ibn_invoice_grouping.sql) to:
--   CREATE branch: also insert this group's computed Dispatch-Reco rows
--     (erp_production.dispatch_reco), same transaction as the P601 posting
--     and the invoice/lines/additional-cost-lines writes (§8D).
--   REVERSE branch: also void (is_voided=true/voided_at, never delete --
--     matches PR19/COR6's established append-on-reversal pattern) every
--     Dispatch-Reco row tied to the invoice being reversed.
