-- Section 30 (ACL_SSOT.md) — propagates menu_visible into precomputed_acl_view.
--
-- OR-reduce rule (§30.5, LOCKED): menu_visible must be OR'd across every
-- ALLOW-contributing source for the same (auth_user_id, work_context_id,
-- resource_code, action_code) key — capability grants (which carry a real
-- menu_visible flag), role_menu_permissions grants, and user_overrides
-- (neither of which has a menu_visible column, so they contribute an
-- implicit TRUE) — never just the single winning row that resolved_base's
-- precedence ROW_NUMBER() happens to keep. A narrow, lookup-only capability
-- can only narrow *itself*; it can never suppress a page another one of the
-- user's own grants (role-based or override-based) legitimately wants visible.
--
-- Nothing else in the function's precedence/decision logic changes.

ALTER TABLE acl.precomputed_acl_view
  ADD COLUMN menu_visible boolean NOT NULL DEFAULT true;

CREATE OR REPLACE FUNCTION acl.generate_acl_snapshot(p_acl_version_id uuid, p_company_id uuid)
 RETURNS void
 LANGUAGE plpgsql
...
BEGIN
  ...
  capability_candidates AS (
    SELECT
      ...
      vcma.menu_visible AS menu_visible
  ...
  base_candidates AS (
    SELECT auth_user_id, work_context_id, department_id, resource_code, action_code,
           decision, decision_reason, source_role_rank, precedence_weight
  ...
  -- §30.5 OR-reduce: every ALLOW-contributing source for a given key votes on
  -- visibility. Capability grants carry a real flag; role_menu_permissions and
  -- user_overrides have none, so they always vote TRUE (they were never the
  -- mechanism this narrowing exists for).
  menu_visibility_votes AS (
    SELECT auth_user_id, work_context_id, resource_code, action_code, menu_visible
    FROM capability_candidates
    WHERE decision = 'ALLOW'

    UNION ALL

    SELECT auth_user_id, work_context_id, resource_code, action_code, TRUE AS menu_visible
    FROM role_candidates
    WHERE decision = 'ALLOW'
  ...
  menu_visibility_agg AS (
    SELECT
      auth_user_id, work_context_id, resource_code, action_code,
      BOOL_OR(menu_visible) AS menu_visible
    FROM menu_visibility_votes
    GROUP BY auth_user_id, work_context_id, resource_code, action_code
  )
  ...
  SELECT
    ...
    COALESCE(mva.menu_visible, TRUE) AS menu_visible
  FROM final_ranked
  LEFT JOIN menu_visibility_agg AS mva
    ON mva.auth_user_id = final_ranked.auth_user_id
   AND mva.work_context_id = final_ranked.work_context_id
   AND mva.resource_code = final_ranked.resource_code
   AND mva.action_code = final_ranked.action_code
