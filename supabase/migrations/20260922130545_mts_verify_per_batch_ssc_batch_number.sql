-- §138 (2026-09-22, business owner): MTS Verify's TypeScript side now posts one P261/P101
-- per batch (its own batch_number) instead of one blended lump sum per material, so
-- stock_ledger gains per-batch genealogy -- matches MTO/HPS/MTEST's existing convention
-- (§119.7), while stock_snapshot stays blended either way (unchanged, §8D). This migration
-- only updates the QA-hold audit write inside complete_process_po_verify's MTS branch to
-- carry that same batch_number through to stock_status_change_posting (a column that
-- already existed there but was always written as NULL for MTS) -- no other behavior
-- changes; the non-MTS branch is untouched.
