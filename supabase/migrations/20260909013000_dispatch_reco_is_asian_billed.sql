-- §135.12 (AC10 Reco Data) -- dispatch_reco's own "Shape 2" RPS passthrough
-- (computeDispatchRecoRows() in do_unified.handlers.ts) only ever wrote a
-- row when the SO's dispatch_type identified the Bill-To as Asian Paints
-- (ASIAN_BILLED_DISPATCH_TYPES). A plain INDEPENDENT_PARTY RM/PM/INT sale
-- (not billed to Asian Paints) got NO dispatch_reco row at all -- confirmed
-- live, 2026-09-09: CMP006 invoice 9200000007 (5,290 KG ESR, dispatch_type
-- INDEPENDENT_PARTY) has zero dispatch_reco rows, even though it's a real,
-- POSTED RM sale. Business owner: AC10 should be able to show ALL RM sale,
-- not just the AP-billed subset -- with a togglable "Exclude non-AP-billed"
-- filter, not a silent omission.
--
-- This column lets a row say which case it is, without a fresh
-- sales_order.dispatch_type join every time it's read: DEFAULT true is
-- correct for every pre-existing row -- Shape 1 (batch-linked MTO/HPS/MTEST
-- dispatch) only ever exists for Asian Paints' own FG production, and the
-- OLD Shape 2 gate meant every row that already exists today passed the
-- Asian-billed check to get written at all.
ALTER TABLE erp_production.dispatch_reco
  ADD COLUMN is_asian_billed boolean NOT NULL DEFAULT true;

-- Shape 2 passthrough now fires for EVERY RPS dispatch (Asian-billed or
-- not) -- the old gate (`isAsianBilled`) moves from "write the row at all"
-- to just this one column's value. ap_approved_qty stays populated the
-- same way for both cases (a straight passthrough sale has nothing to
-- separately "approve" either way -- the dispatched quantity IS the
-- reconciled quantity, Asian Paints or not); Standard/Actual stay NULL for
-- both, matching the existing Shape 2 rule.
...
BEGIN
  IF v_action = 'CREATE' THEN
    ...
    -- fully computed (2-level ratio or simple RPS passthrough) by
    -- do_unified.handlers.ts's computeDispatchRecoRows() before this call --
    -- this is a pure write, no derivation happens in SQL. is_asian_billed
    -- (§135.12) defaults true if the payload ever omits it, matching every
    -- pre-existing row's own real status.
