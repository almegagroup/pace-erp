-- §133.21 -- same display-only rate snapshot as delivery_challan_line
-- (20260901130000), now also carried onto the permanent posted invoice
-- line, so a posted MTEST/Pack-UoM invoice keeps showing "Rate x / Per
-- BBL" instead of reverting to the per-KG figure once viewed after posting.
-- rate/uom_code/taxable_value/line_total (the actual math) are unchanged.
...
-- Extends complete_pgi_invoice_action() (last touched by
-- 20260828170000_dispatch_reco_write_and_revoke_void.sql) purely to persist
-- the three new columns on the CREATE branch's sales_invoice_line insert.
-- No other statement in this function changes.
...
BEGIN
  IF v_action = 'CREATE' THEN
    ...
    INSERT INTO erp_procurement.sales_invoice_line (
      ...
      display_rate_basis, display_rate, display_uom_code,
