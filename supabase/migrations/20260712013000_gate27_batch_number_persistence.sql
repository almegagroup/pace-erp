/*
 * File-Path: supabase/migrations/20260712013000_gate27_batch_number_persistence.sql
 * Domain: INVENTORY / PRODUCTION
 * Purpose: Replace dead stock_ledger.batch_id with batch_number persistence and
 *          extend post_stock_movement() overloads with a backward-compatible
 *          optional p_batch_number parameter.
 * Authority: Backend
 */
...
BEGIN
  ...
  INTO v_has_batch_id_column;

  IF v_has_batch_id_column THEN
    SELECT EXISTS (
      SELECT 1
      ...
      WHERE tc.constraint_type = 'FOREIGN KEY'
        AND tc.table_schema = 'erp_inventory'
        AND tc.table_name = 'stock_ledger'
        AND kcu.column_name = 'batch_id'
    )
    INTO v_has_fk;

    EXECUTE 'SELECT COUNT(*) FROM erp_inventory.stock_ledger WHERE batch_id IS NOT NULL'
      INTO v_non_null_rows;

    IF v_has_fk THEN
      RAISE EXCEPTION 'Refusing to drop erp_inventory.stock_ledger.batch_id: foreign key still exists';
    END IF;

    IF v_non_null_rows <> 0 THEN
      RAISE EXCEPTION 'Refusing to drop erp_inventory.stock_ledger.batch_id: % non-null row(s) still exist', v_non_null_rows;
    END IF;

    DROP INDEX IF EXISTS erp_inventory.idx_sl_batch;
    ALTER TABLE erp_inventory.stock_ledger DROP COLUMN IF EXISTS batch_id;
  END IF;
END $$;
...
DROP FUNCTION IF EXISTS erp_inventory.post_stock_movement(
  text, date, date, text, uuid, uuid, uuid, numeric, text, numeric, text, text, uuid, uuid
);
...
DROP FUNCTION IF EXISTS erp_inventory.post_stock_movement(
  text, date, date, text, uuid, uuid, uuid, uuid, numeric, text, numeric, text, text, uuid, uuid
);
...
BEGIN
  ...
  INSERT INTO erp_inventory.stock_ledger (
    stock_document_id, posting_date, company_id, plant_id, storage_location_id, material_id, batch_number,
    stock_type_code, movement_type_code, direction, quantity, base_uom_code, value,
    valuation_rate, created_by
  ) VALUES (
    v_stock_doc_id, p_posting_date, p_company_id, p_plant_id, p_storage_location_id, p_material_id, p_batch_number,
    p_stock_type_code, p_movement_type_code, p_direction, p_quantity, p_base_uom_code,
    p_quantity * p_unit_value, p_unit_value, p_posted_by
  )
  RETURNING id INTO v_ledger_id;
...
REVOKE EXECUTE ON FUNCTION erp_inventory.post_stock_movement(
  text, date, date, text, uuid, uuid, uuid, uuid, numeric, text, numeric, text, text, uuid, uuid, text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION erp_inventory.post_stock_movement(
  text, date, date, text, uuid, uuid, uuid, uuid, numeric, text, numeric, text, text, uuid, uuid, text
) TO service_role;
