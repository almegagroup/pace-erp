-- Update save_ac01_grn_cost to respect the new has_gst gate (2026-08-21):
-- GST is only backed out of a cost line's total when has_gst=true AND
-- gst_treatment='INCLUSIVE'. has_gst=false (no GST relevance at all) and
-- has_gst=true+EXCLUSIVE both use the full amount as-is — same math as
-- before, but has_gst=false is now the honest, explicit way to say "this
-- charge carries no GST", instead of overloading EXCLUSIVE for it.
-- Also persists has_gst on the inserted landed_cost_line row.
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO erp_procurement.landed_cost_line (
      ...
      entry_mode, percentage, has_gst, gst_treatment, gst_rate
    ) VALUES (
