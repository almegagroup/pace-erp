-- Carry the new display_rate_basis/display_rate/display_uom_code columns
-- (20260901130000) through the DO Create/Edit atomic RPC. Presentation-only
-- -- unit_value/quantity/gst_amount/line_total (the actual math) are
-- unchanged and still computed the same way in TypeScript before this call.
...
BEGIN
  ...
  LOOP
    INSERT INTO erp_procurement.delivery_challan_line (
      ...
      nullif(v_line->>'display_uom_code', '')
    );
