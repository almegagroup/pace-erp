...
### 2026-09-12 — SO01 FG SKU type-first search latency correction

- **Root cause:** Opening a remote SO01 SKU picker issued an empty-term request, then the generic combobox automatically requested further cursor pages whenever the dropdown was short. Each page performs pack, prodshade/configuration, stroke/applicability and UOM-conversion eligibility checks. The observed 17 browser requests were therefore repeated full-master eligibility work, not an absent SKU or a database pagination limit.
- **Code fix:** SO01 now waits for three typed characters before querying; the API independently returns an empty result for blank/short terms so another client cannot reintroduce the full scan. Remote comboboxes wait for an actual scroll or keyboard navigation before requesting another cursor page. Within one bounded typed page, independent pack/prodshade/conversion lookups and independent configuration/stroke lookup batches now run in parallel. Eligibility, own-company priority, cross-company fallback, shared-stroke PO-type rules, MTEST rules and conversion selection remain unchanged.
