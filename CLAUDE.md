...
**➡️ Sequencing locked (2026-07-11, user decision):** Packing PO (full Standard→Final cycle, no Verify — Verify is Process PO only) → **then** Opening Stock — **all of it together (RM + PM + FG), not FG alone** (Liquid, 1 July go-live, per §83.2 session decision) → **then** Dispatch module (L5). Do not jump ahead of this order.
...
**🔴 Gate-27 costing/batch discovery session (2026-07-12) — Batch Number Persistence LOCKED, 4 more issues open:**
Deep-dive into how HPS/MTO's batch-level costing/dispatch/salvage actually works surfaced a real, previously-unknown engine gap: `post_stock_movement()` had no batch parameter at all, and `stock_ledger.batch_id` was dead (no FK, never written). Fixed by design — see feasibility §83.7 "Batch Number Persistence Mechanism" for the full locked schema/function/call-site design (not yet implemented in code). Four more issues surfaced in the same session, still open:
- Partial batch-keyed reversal for salvage/excess (not full CORS) — math already locked (§104.7 Reversal Basis), trigger/UI mechanism not designed
- Partial return + pack-type-change workflow (e.g. tanker return, partial qty, repack into barrels) — math already locked (§104.7 Return Receipt), workflow sequencing not designed
- Salvage-blend granular RM decomposition (Source Batch + Blend Qty auto-derives RM contribution via Actual RM Ratio, added as New Row entries) — principle confirmed in chat, not yet written into feasibility doc
- Reusable vs single-use PM flag (barrel/IBC = recoverable on reprocess, label/tape/seal = not) — no such flag exists on `material_master` today, needs one added
- MTS/INT/MTEST explicitly deferred — this whole batch-persistence effort is MTO/HPS-scoped only, do not extend without a dedicated session
