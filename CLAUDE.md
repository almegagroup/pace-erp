...
- **Packing PO PM AP-Reco derivation report — blocked on Dispatch (L5) (2026-07-21).** §83.4.1-addendum built the *data capture* side: `packing_order_line_reco` (PM, new table) + `process_order_line_reco` (RM/INT, pre-existing) now both correctly accumulate Actual/AP-Approved/Variance. **Not yet built:** the query/report that joins them into one AP Reco number for a dispatch — `ratio = qty this Packing PO drew ÷ Process PO batch's total actual output`, then `AP Reco = ratio × process_order_line_reco.ap_approved_qty` (RM/INT) `+ packing_order_line_reco.ap_approved_qty` (PM). The ratio's numerator ("Dispatch Qty") is a Dispatch-module concept that doesn't exist yet, so this report is correctly sequenced **after** the Dispatch (L5) formal session, not before — business owner confirmed 2026-07-21. Do not build this derivation early against a guessed Dispatch shape.
