...
- **✅ SAP-Identical Multi-UoM — DESIGN LOCKED 2026-07-24 (Section 110), IMPLEMENTATION IN PROGRESS.** Business owner challenge: "user 3 UoM-এ (Base/Middle/Dispatch) stock দেখতে ও কাজ করতে পারবে, SAP-এর মতোই, কোনো পার্থক্য না।" Verified the foundation already exists — `pack_code_master.outer_uom_code` (FG dispatch unit, no new column needed), `material_uom_conversion` (generic conversion storage, already working via Pack BOM sync) — what's missing is **wiring**: no transaction screen anywhere (GRN, PO, future Dispatch) lets a user enter/view quantity in an alternate unit; `purchase_uom_code`/`issue_uom_code` exist on `material_master` but are set on only 3 of ~92 RM/PM materials and read by zero screens. **Locked: graceful fallback** — no bulk data-entry prerequisite; a material with no alternate unit defined just shows base UoM (today's behavior), no code change needed per-material once the mechanism exists. 5-phase plan: **A** (today) reusable UoM Quantity Picker (new procurement-domain `assertProcurementReadRole`-gated endpoint — deliberately not reusing OM's manager-only one, since L1/L2 procurement staff create GRN/PO — + generic frontend component), **B** (today) retrofit GRN/PO line entry, **C** (today) alternate-unit column on stock reports (FG Stock Breakdown first), **D** (Physical Inventory/L7 formal session, not yet scheduled) count-in-dispatch-UoM per MI04, **E** (Dispatch/L5 formal session, not yet scheduled) dispatch entry in pack units. D/E must reuse Phase A's component, not reinvent it.
