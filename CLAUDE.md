...
- **✅ Opening Rate "Recalculate" mechanism — DESIGN LOCKED + Phase 1 IMPLEMENTED (2026-07-24).** Full design: feasibility doc **Section 109**. Business owner confirmed full replay (not a shortcut `stock_snapshot` overwrite), including PR19/COR6/CORS postings, **and** full RM→SFG→FG cascade. Given the size/risk, split into 3 phases (mirrors §107.8's staged rollout): **Phase 1 (✅ done)** — `erp_inventory.recalculate_valuation()` (migration `20260724100000`) replays one material's own `stock_ledger` history chronologically from a corrected opening rate, updates `stock_snapshot.valuation_rate`, logs to new `valuation_correction_log` table, and returns an `impacted_rows` list (OUT-direction legs whose "value removed" changes under the corrected rate) — this does NOT cascade yet, just computes+logs what Phase 2/3 will need. Verified against live Dev data (Tartaric Acid) in a rolled-back transaction — math confirmed by hand. Backend: `recalculateValuationHandler` in `opening_stock.handlers.ts` (manager/SA gated) + route + ACL (`PROC_OPENING_STOCK_APPROVAL`/APPROVE). Frontend: IN05 (`OpeningStockDetailPage.jsx`) — bulk table (per-line "Corrected Rate" input + one shared Reason + one "Recalculate All" button, parallel per §8B), not one-click-per-line (corrected same day per business owner feedback — original UX draft was too clicky). **One-time-use enforced server-side** (migration `20260724110000`) — `recalculate_valuation()` blocks a second call on the same (company, material, location, stock_type) with `VALUATION_RECALC_ALREADY_DONE`; UI shows a "Recalculated" badge instead of the input once done; no "reopen" mechanism exists yet (deliberately deferred). **Phase 2 (SFG cascade) and Phase 3 (FG cascade) not started** — Phase 1 must be live-verified first since 2/3 recursively reuse its engine.
