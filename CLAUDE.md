...
**✅ Sales Module Stage 3 (PGI + Invoice) — DESIGN LOCKED 2026-07-31 (feasibility §113.13-113.15), IMPLEMENTATION MOSTLY PENDING.** Scope: RM/PM/INT only (FG Phase 2, untouched). Live-code audit found the legacy Sales Invoice mechanism (SO02, `sales_invoice`/`sales_invoice_line`) blocks STO entirely (`dc_type=SALES` hard requirement) and `postSalesInvoiceHandler` never calls `post_stock_movement()` at all — PGI today only happens via the old pre-DO atomic SO issue-stock path, unrelated to Invoice. **Locks:** (1) DO itself will carry a full commercial snapshot (rate/GST/packaging-cost/freight/rebate, copied from SO/STO line at DO-create time — safe because that line freezes the moment a DO exists against it) — schema not yet added (§113.13). (2) GST CGST+SGST-vs-IGST bug fixed for SO: was comparing GSTIN state-code prefixes, which always fell through to IGST for an unregistered customer (no GSTIN) even if same-state — **✅ implemented** (commit `3a1fe949`): new `erp_master.customer_master.billing_state` (mandatory, mirrors `companies.state_name`), `deriveSalesInvoiceGstType()` now compares state names directly. STO's equivalent (sending vs receiving `companies.state_name`, no new field needed) still pending. (3) One unified PGI+Invoice mechanism for **both** SO and STO — reuses the existing SO02 page/table (`sales_invoice`), not a separate STO page; the SALES-only block gets removed. (4) 1 DO = 1 Invoice always. (5) New mandatory Tally Invoice Number + Date fields (ERP's own invoice is tracking-only, no IRN/e-invoice link — real legal invoice is in Tally). (6) SO02 becomes a DO list / PGI queue (100/page, not-yet-PGI'd rows on top) with a new "PGI & INVOICE" button → DO picker → auto-resolved fields (state, SO/STO number, consignee, transporter, LR, GST breakup, packaging cost) + manual Tally fields + freight Yes/No (only if freight_term is exclusive) → read-only review screen → final submit posts stock movement **P601** "GI for Dispatch (Delivery)" (already existed in `movement_type_master`, never used by any code) per line, generates the Invoice number, sets `delivery_challan.status = 'DISPATCHED'` (constraint already allowed this value, never set by code). (7) Reversal: DO can be cancelled whole (not per-line, reason required) before PGI, releasing its `reservation_document`; post-PGI reversal cancels the Invoice, reverses stock via **P602**, and releases the DO back to the pending queue for a fresh Invoice attempt (Tally number/date reuse on the retry is fine, no uniqueness enforced). Full detail in feasibility §113.15 — read it before implementing.
