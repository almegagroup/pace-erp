...
- 🔶 **Issue 3 — Partial return + pack-type-change workflow** (e.g. tanker return, partial qty, repack into barrels) — math already locked (§104.7 Return Receipt), workflow sequencing still not designed. May turn out to be a thin wrapper around PR19 (reverse via PR19, then create the new-pack-type Packing PO) rather than a separate mechanism — worth checking against PR19's final shape before designing from scratch.
