...
- ~~FG Costing System (83.13 → Section 104) — dedicated session deferred ~1 week~~ → **🔴 2026-07-17: DESIGN LOCKED (§104.8 + §104.9), NOT YET IMPLEMENTED, and it is a 1 JULY GO-LIVE BLOCKER.** Cost = RMC + PMC + Conversion (all per-KG, × pack size); SFG = RMC + Conversion, FG = SFG + PMC. Conversion rate config = segment default + Prodshade override, **`valid_from` dated (no `valid_to` stored — derived as next valid_from−1)**, resolved by posting_date, hard-block if no rate. Opening MTO/HPS batches get a synthetic **"Opening Process PO"** (§104.9) so PR19/Reco/Return work unchanged — its RM/PM lines **must never call `post_stock_movement()`** (genealogy only, RM/PM stock must not move); `source_txn_type='OPENING'`; PR19 must tolerate `reversalOfId = NULL` for these.
