...
export default function OrderListPage() {
  ...
  const processOrders = categoryFilteredProcessOrders.filter((order) => {
    ...
    const haystack = [
      ...
      order.batch_number, order.status, order.created_at?.slice(0, 10),
  ...
  const packingOrders = categoryFilteredPackingOrders.filter((order) => {
    ...
    const haystack = [
      ...
      order.material?.external_code, order.pack_code?.pack_code, order.num_packs, order.fill_qty_per_pack,
      order.planned_qty_kg, order.actual_qty_kg, order.process_order?.po_number, order.process_order?.batch_number,
      order.status,
    ].filter(Boolean).join(" ").toLowerCase();
    return haystack.includes(searchNeedle);
  });
