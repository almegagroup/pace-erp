...
export default function SlocCostingGroupPage() {
  ...
  return <ErpScreenScaffold title="Monthly Costing Rate Workspace" subtitle="AC06 | company-scoped Dispatch rate control" actions={[{ label: "Refresh", tone: "secondary", onClick: () => { void refresh(); } }, ...(canClose ? [{ label: "Close Month", tone: "danger", disabled: busy || workspace.month?.status === "CLOSED" || !companyId, onClick: () => { if (window.confirm(`Close ${month}? This creates an immutable archive.`)) void withBusy(() => closeAc06Month({ company_id: companyId, rate_month: month })); } }] : [])]}>
    ...
    <div className="flex flex-wrap gap-2"><Chip label="Month" value={workspace.month?.status || "OPEN"} tone={workspace.month?.status === "CLOSED" ? "plain" : "ok"} /><Chip label="Access" value={canSetup || canRate || canVerify || canClose ? "Maintenance" : "View only"} /><Chip label="Rows" value={summary.rows || 0} /><Chip label="Verified" value={summary.verified || 0} tone="ok" /><Chip label="Pending" value={summary.pending || 0} tone="warn" /><Chip label="Standalone" value={summary.standalone || 0} /></div>
    ...
    {tab === "rate" ? <ErpSectionCard title="Monthly Costing Rate Input"><div className="mb-3 flex flex-wrap justify-between gap-2"><p className="text-xs text-slate-600">Changing a rate immediately makes its scope pending again. Only pending standalone rows and Costing Group lead rows can be verified.</p><div className="flex gap-2">{canVerify ? <button type="button" disabled={busy || !selectionValid} onClick={() => void withBusy(() => verifyAc06Rates({ company_id: companyId, rate_month: month, line_ids: selectedForVerify }))} className="border border-emerald-700 bg-emerald-50 px-3 py-1 text-xs font-semibold disabled:opacity-40">Verify Selected</button> : null}{canRate ? <button type="button" disabled={busy || !selectedSlocRows.length || workspace.month?.status === "CLOSED"} onClick={() => void withBusy(() => saveAc06Rates({ company_id: companyId, rate_month: month, updates: selectedSlocRows.filter((row) => row.is_standalone || row.is_group_lead).map((row) => ({ line_id: row.id, rate: rateDraft[row.id] ?? row.rate })) }))} className="border border-sky-700 bg-sky-100 px-3 py-1 text-xs font-semibold">Save Rates</button> : null}</div></div><ErpDenseGrid columns={[{ key: "verify", label: "Verify", render: (row) => <input type="checkbox" disabled={!canVerify || !pendingIds.has(row.id) || workspace.month?.status === "CLOSED"} checked={selectedForVerify.includes(row.id)} onChange={() => toggle(selectedForVerify, row.id, setSelectedForVerify)} /> }, { key: "pace_code", label: "Material", render: (row) => `${row.pace_code || "-"} | ${row.material_name || "-"}` }, { key: "costing_group_name", label: "Costing Group", render: (row) => row.costing_group_name || "Standalone" }, { key: "rate", label: "Rate", render: (row) => canRate && workspace.month?.status !== "CLOSED" && (row.is_standalone || row.is_group_lead) ? <input value={rateDraft[row.id] ?? row.rate ?? "0"} onChange={(event) => setGroupRate(row, event.target.value)} inputMode="decimal" className="h-8 w-32 border border-slate-300 px-2 font-mono" /> : <span className="font-mono">{rateDraft[row.id] ?? row.rate ?? "0"}</span> }, { key: "verification_status", label: "Status" }, { key: "lead", label: "Entry", render: (row) => row.is_standalone ? "Standalone" : row.is_group_lead ? "Group Lead" : "Auto-filled" }]} rows={selectedSlocRows} rowKey={(row) => row.id} emptyMessage="Create a SLOC Group first, then select it to load eligible items." /></ErpSectionCard> : null}
    {tab === "sloc" ? <ErpSectionCard title="SLOC Group Setup"><div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)]"><div className="grid gap-3 border border-slate-200 bg-white p-4"><h3 className="text-sm font-semibold text-slate-900">{editingSlocId ? "Edit SLOC Group" : "New SLOC Group"}</h3><p className="text-xs leading-5 text-slate-500">Choose the company storage locations that belong to this AC06 costing scope.</p><label className="grid gap-1 text-sm text-slate-700"><span className="font-medium text-slate-800">Group Name</span><input value={slocName} onChange={(event) => setSlocName(event.target.value)} className="h-10 border border-slate-300 px-3 outline-none focus:border-sky-500" /></label><label className="grid gap-1 text-sm text-slate-700"><span className="font-medium text-slate-800">Storage Locations</span><select multiple value={slocLocations} onChange={(event) => setSlocLocations(Array.from(event.target.selectedOptions).map((option) => option.value))} className="min-h-48 border border-slate-300 px-2 py-2 outline-none focus:border-sky-500">{(locationsQuery.data ?? EMPTY).map((location) => <option key={location.id} value={location.id}>{`${location.code || location.storage_location_code || "-"} - ${location.name || location.storage_location_name || "Unnamed"}`}</option>)}</select></label><div className="flex gap-2"><button type="button" disabled={busy || !slocName || !slocLocations.length} onClick={() => void withBusy(async () => { if (editingSlocId) await updateAc06SlocGroup(editingSlocId, { company_id: companyId, group_name: slocName, storage_location_ids: slocLocations }); else await createAc06SlocGroup({ company_id: companyId, group_name: slocName, storage_location_ids: slocLocations }); setSlocName(""); setSlocLocations([]); setEditingSlocId(""); })} className="border border-sky-700 bg-sky-100 px-3 py-2 text-sm font-semibold text-sky-950 disabled:cursor-not-allowed disabled:opacity-40">{editingSlocId ? "Save Group" : "Save Group"}</button><button type="button" onClick={() => { setEditingSlocId(""); setSlocName(""); setSlocLocations([]); }} className="border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700">{editingSlocId ? "Cancel" : "Clear"}</button></div></div><ErpDenseGrid columns={[{ key: "group_name", label: "Existing SLOC Group" }, { key: "member_count", label: "SLOCs", render: (row) => (row.storage_location_ids || []).length }, { key: "active", label: "Status", render: () => "Active" }, { key: "actions", label: "Action", render: (row) => <div className="flex gap-2"><button type="button" onClick={() => { setEditingSlocId(row.id); setSlocName(row.group_name); setSlocLocations((row.storage_location_ids || []).map(String)); }} className="text-xs font-semibold text-sky-800">Edit</button><button type="button" onClick={() => { if (window.confirm(`Delete ${row.group_name}?`)) void withBusy(() => deleteAc06SlocGroup(row.id, { company_id: companyId })); }} className="text-xs font-semibold text-rose-800">Delete</button></div> }]} rows={slocGroups} rowKey={(row) => row.id} emptyMessage="No AC06 SLOC Group exists for this company yet." /></div></ErpSectionCard> : null}
