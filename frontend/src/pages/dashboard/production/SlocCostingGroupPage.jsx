...
function Chip({ label, value, tone = "plain" }) {
  const colors = {
    plain: "border-slate-300 bg-white",
    ok: "border-emerald-300 bg-emerald-50",
    warn: "border-amber-300 bg-amber-50",
  };
  return (
    <span
      className={`rounded-full border px-3 py-1 text-xs font-semibold text-slate-700 ${colors[tone]}`}
    >
      {label}: {value}
    </span>
  );
...
export default function SlocCostingGroupPage() {
  ...
  const reportMonthList = (searchParams.get("months") || reportMonths)
    .split(",")
  ...
  const reportQuery = useQuery({
    ...
    queryFn: () =>
      getAc06Report({
        company_id: companyId,
        sloc_group_id: reportSlocId,
        months: reportMonthList.join(","),
      }),
    enabled:
      reportMode &&
      Boolean(companyId && reportSlocId && reportMonthList.length),
    select: unwrap,
  });
  ...
  const visibleRateRows = selectedSlocRows.filter(
    (row) =>
      ...
      `${row.pace_code || ""} ${row.material_name || ""} ${row.costing_group_name || ""}`
        .toLowerCase()
  ...
  if (reportMode) {
    ...
    return (
      <ErpScreenScaffold
        ...
        subtitle="AC06 full-page, multi-month comparison"
      >
        <ErpSectionCard title="Report Scope">
          <div className="grid gap-3 md:grid-cols-3">
            <ErpFieldPreview label="Company" value={companyId || "-"} />
            <ErpFieldPreview label="SLOC Group" value={reportSlocId || "-"} />
            <ErpFieldPreview
              label="Months"
              value={reportMonthList.join(", ")}
            />
          </div>
        </ErpSectionCard>
        <ErpDenseGrid
          columns={[
            ...
            { key: "month_status", label: "Month Status" },
          ]}
          rows={reportRows}
          rowKey={(row) => row.id}
  ...
  return (
    <ErpScreenScaffold
      ...
      subtitle="AC06 | company-scoped Dispatch rate control"
      actions={[
        {
          label: "Refresh",
          tone: "secondary",
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        <Chip
          ...
          tone={workspace.month?.status === "CLOSED" ? "plain" : "ok"}
        />
        <Chip
          label="Access"
          value={
        ...
        />
        <Chip label="Rows" value={summary.rows || 0} />
        <Chip label="Verified" value={summary.verified || 0} tone="ok" />
        <Chip label="Pending" value={summary.pending || 0} tone="warn" />
        <Chip label="Standalone" value={summary.standalone || 0} />
        <Chip label="Excluded" value={summary.excluded || 0} />
      </div>
      ...
      {tab === "dashboard" ? (
        <ErpSectionCard title="Costing Dashboard">
          <div className="grid gap-4">
            <p className="text-sm text-slate-600">
              Choose Company, SLOC Group, and one or more months to open the
              full-page rate comparison. The report can be opened in a separate
              ERP window through Shift+F8.
            </p>
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <button
                ...
                onClick={() =>
                  window.open(
                    `/dashboard/production/sloc-costing-group?report=1&sloc_group_id=${encodeURIComponent(slocGroupId)}&months=${encodeURIComponent(reportMonths)}`,
                    "_blank",
                    "noopener,noreferrer",
                  )
                }
                className="border border-sky-700 bg-sky-100 px-4 text-sm font-semibold"
              >
                Execute Full Report
              </button>
            </div>
      ...
      {tab === "rate" ? (
        <ErpSectionCard title="Monthly Costing Rate Input">
          <div className="mb-3 flex flex-wrap justify-between gap-2">
            <p className="text-xs text-slate-600">
              Changing a rate immediately makes its scope pending again. Only
              pending standalone rows and Costing Group lead rows can be
              verified.
            </p>
          ...
          <div className="mb-3 grid gap-3 md:grid-cols-[minmax(0,1fr)_240px_auto]">
            <input
              value={rateSearch}
      ...
      {tab === "sloc" ? (
        <>
          <ErpSectionCard title="SLOC Group Setup">
            <div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
              <div className="grid gap-3 border border-slate-200 bg-white p-4">
                ...
                <p className="text-xs leading-5 text-slate-500">
                  Choose the company storage locations that belong to this AC06
                  costing scope.
                </p>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {editingSlocId ? "Save Group" : "Save Group"}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
      ...
      {tab === "group" ? (
        <ErpSectionCard title="Costing Group Setup">
          <div className="grid gap-4 border border-slate-200 p-4">
            ...
            <ErpDenseGrid
              columns={[
                ...
                { key: "verification_status", label: "Rate Status" },
              ]}
              rows={groupSetupRows}
              rowKey={(row) => row.id}
            ...
            <ErpDenseGrid
              columns={[
                { key: "group_name", label: "Costing Group" },
                {
                  ...
                  render: (row) =>
                    slocGroups.find((group) => group.id === row.sloc_group_id)
                      ?.group_name || "-",
                },
                {
                  key: "actions",
      ...
      {tab === "history" ? (
        <ErpSectionCard title="History / Archive">
          <p className="mb-3 text-sm text-slate-600">
            Closed snapshots are immutable. The selected month is shown below;
            open the full-page report to compare multiple months.
          </p>
          <ErpDenseGrid
            columns={[
              { key: "material_code_snapshot", label: "Material" },
              {
                key: "costing_group_name_snapshot",
                label: "Costing Group",
                render: (row) =>
