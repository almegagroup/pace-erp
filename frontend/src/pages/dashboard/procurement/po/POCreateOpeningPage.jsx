...
export default function POCreateOpeningPage() {
  ...
  async function handleSubmit() {
    ...
    if (!form.po_number.trim()) {
      setError("Legacy PO number is required.");
      return;
    }
    if (!form.po_date) {
    ...
    try {
      ...
      const createdOrders = Array.isArray(created?.purchase_orders)
        ? created.purchase_orders
        : created?.id
          ? [created]
          : [];
      const confirmResults = await Promise.allSettled(
        createdOrders.map((po) => confirmPurchaseOrder(po.id, {}))
      );
      const succeeded = [];
      const failed = [];

      confirmResults.forEach((result, index) => {
        const po = createdOrders[index];
        const line = po?.lines?.[0];
        const materialLabel = line?.material_name || line?.material_id || `Line ${index + 1}`;
        const descriptor = `${po?.po_number || form.po_number.trim()} (${materialLabel}) [${po?.id || "no-id"}]`;
        if (result.status === "fulfilled") {
          succeeded.push(descriptor);
          return;
        }
        const reason = result.reason instanceof Error ? result.reason.message : "PROCUREMENT_PO_CONFIRM_FAILED";
      ...
      if (failed.length > 0) {
        setNotice(
          succeeded.length > 0
            ? `${succeeded.length} opening PO line(s) created and confirmed successfully.`
            : ""
        );
        setError(`Opening PO batch partially confirmed. Failed: ${failed.join(" | ")}`);
        return;
      }
          ...
          <div className="grid gap-4">
            <ErpSectionCard eyebrow="PO Header" title="Vendor and dispatch basics">
              <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-4">
                <label className="grid gap-1 text-xs font-semibold text-slate-700">
                  Legacy PO Number <span className="text-rose-500">*</span>
                  <input
                    value={form.po_number}
                    onChange={(event) => updateHeaderField("po_number", event.target.value)}
                    placeholder="Enter legacy PO number"
                ...
                <label className="grid gap-1 text-xs font-semibold text-slate-700">
                  ...
                  <input
                    type="date"
                    value={form.po_date}
                    onChange={(event) => updateHeaderField("po_date", event.target.value)}
